    ...
    <header className="sticky top-0 z-50 border-b border-black/6 bg-white/85 backdrop-blur-2xl">
      <div className="mx-auto flex min-h-[76px] max-w-7xl items-center justify-between gap-5 px-5 lg:px-8">
        ...
        <nav
          ...
          className={`${open ? 'absolute left-4 right-4 top-[70px] flex' : 'hidden'} flex-col gap-1 rounded-3xl border border-black/6 bg-white/95 p-3 shadow-[0_25px_70px_rgba(16,17,20,.12)] lg:static lg:flex lg:flex-row lg:items-center lg:gap-1 lg:border-0 lg:bg-transparent lg:p-0 lg:shadow-none`}
        >
          {NAV_ITEMS.slice(0, 1).map((item) => (
            <Link key={item.to} to={item.to} aria-current={isRouteActive(location.pathname, item.to) ? 'page' : undefined} className="nav-link rounded-xl px-3 py-3 text-sm font-semibold text-neutral-600 hover:text-[#e3262e] lg:px-3 lg:py-2">
              {item.label}
            </Link>
          ))}
          ...
          <div ref={solutionRef} className="relative">
            ...
            >
              Soluções
              <ChevronDown size={15} className={`transition-transform duration-200 ${solutionsOpen ? 'rotate-180' : ''}`} />
            </button>

            {solutionsOpen && (
              <div role="menu" className="mt-1 grid gap-1 rounded-2xl border border-black/6 bg-white p-2 shadow-[0_25px_70px_rgba(16,17,20,.14)] lg:absolute lg:left-0 lg:top-12 lg:w-[360px]">
                ...
                {SOLUTION_ITEMS.map((item, index) => (
                  ...
                  >
                    <div className="flex items-center justify-between gap-4">
                      ...
                      <ArrowRight size={15} className="text-red-500 opacity-0 transition group-hover:translate-x-0.5 group-hover:opacity-100" />
                    </div>
                    <span className="mt-1 block text-sm leading-6 text-neutral-500">
                      {index === 0 && 'Gestão comercial, PDV, estoque e financeiro.'}
                      {index === 1 && 'Infrações, Talão Eletrônico e trânsito.'}
                      {index === 2 && 'Assinatura e identidade digital com validade jurídica.'}
                      {index === 3 && 'Instalação, treinamento, manutenção e atendimento.'}
                    </span>
                  </Link>
                ))}
              </div>
          ...
          <Link to="/contato" className="brand-cta mt-2 inline-flex items-center justify-center gap-2 rounded-xl bg-[#e3262e] px-5 py-3 text-sm font-bold text-white lg:ml-2 lg:mt-0">
            Fale conosco
            <ArrowRight size={16} />
          </Link>
        </nav>

        <button type="button" className="grid size-11 place-items-center rounded-xl border border-black/7 bg-white lg:hidden" onClick={() => setOpen((value) => !value)} aria-expanded={open} aria-label={open ? 'Fechar menu' : 'Abrir menu'}>
    ...
    <footer className="bg-[#101114] text-white">
      <div className="mx-auto grid max-w-7xl gap-12 px-5 py-16 md:grid-cols-2 lg:grid-cols-5 lg:px-8">
        <div className="lg:col-span-2">
          <Logo light />
          <p className="mt-6 max-w-md text-base leading-7 text-neutral-400">Tecnologia, gestão e suporte para empresas e órgãos públicos que precisam operar com mais eficiência, segurança e controle.</p>
          <a href={WHATSAPP} target="_blank" rel="noreferrer" className="mt-6 inline-flex items-center gap-2 text-base font-bold text-red-300 hover:text-white">Falar no WhatsApp <ArrowRight size={17} /></a>
        </div>
        <FooterColumn title="Soluções" links={[['SGCI', '/sgci'], ['SISTRANS', '/sistrans'], ['Certificação Digital', '/certificacao-digital'], ['Produtos', '/produtos']]} />
        <FooterColumn title="Empresa" links={[['Sobre a Rede', '/empresa'], ['Parceiros', '/parceiros'], ['Suporte', '/suporte'], ['Contato', '/contato']]} />
        <div>
          <b className="text-xs uppercase tracking-[.16em] text-white/90">Canais</b>
          <div className="mt-5 grid gap-3 text-base text-neutral-400">
            <a href={WHATSAPP} target="_blank" rel="noreferrer" className="hover:text-white">WhatsApp</a>
            <a href="mailto:suporte@redesoftware.com.br" className="hover:text-white">E-mail</a>
            <Link to="/contato" className="hover:text-white">Central de contato</Link>
          </div>
        </div>
      </div>
...
function FooterColumn({ title, links }: { title: string; links: [string, string][] }) {
  return (
    <div>
      <b className="text-xs uppercase tracking-[.16em] text-white/90">{title}</b>
      <div className="mt-5 grid gap-3 text-base text-neutral-400">
...
function PageHero({ eyebrow, title, text, imageSearch, children }: { eyebrow: string; title: ReactNode; text: string; imageSearch?: string; children?: ReactNode }) {
  return (
    <section className="relative overflow-hidden border-b border-black/5 bg-[linear-gradient(180deg,#fff,#fafafa)]">
      <div className="absolute -right-40 -top-32 size-[560px] rounded-full bg-red-100/70 blur-3xl" />
      <div className="absolute left-[-10%] top-48 size-[300px] rounded-full bg-red-50 blur-3xl" />
      <div className="relative mx-auto grid max-w-7xl gap-12 px-5 py-20 lg:grid-cols-[.95fr_1.05fr] lg:items-center lg:px-8 lg:py-28">
        <div>
          <span className="text-xs font-black uppercase tracking-[.22em] text-[#e3262e]">{eyebrow}</span>
          <h1 className="mt-5 max-w-3xl font-display text-5xl font-black leading-[.98] tracking-[-.06em] sm:text-6xl lg:text-7xl">{title}</h1>
          <p className="mt-7 max-w-2xl text-lg leading-8 text-neutral-500 sm:text-xl">{text}</p>
          {children}
        </div>
        {imageSearch ? (
          <div className="relative">
            <div className="absolute -inset-8 rounded-full bg-[radial-gradient(circle,rgba(227,38,46,.14),rgba(227,38,46,0)_68%)] blur-2xl" />
            <div className="relative rounded-[32px] border border-black/8 bg-white p-3 shadow-[0_35px_100px_rgba(16,17,20,.12)]">
              <OriginalMedia search={imageSearch} alt={`${eyebrow} Rede Software`} className="max-h-[500px] w-full rounded-[24px] bg-neutral-100 object-cover" fallback={<FallbackVisual label={eyebrow} />} />
            </div>
          </div>
        ) : null}
      </div>
...
function FallbackVisual({ label, dark = false }: { label: string; dark?: boolean }) {
  return (
    <div className={`grid min-h-[360px] place-items-center rounded-[24px] p-8 text-center ${dark ? 'bg-white/5 text-white' : 'bg-[#101114] text-white'}`}>
      <div>
        <span className={`mx-auto grid size-16 place-items-center rounded-2xl ${dark ? 'bg-white/10 text-red-300' : 'bg-white/10 text-red-300'}`}><Monitor size={28} /></span>
        <p className="mt-5 font-display text-2xl font-black">{label}</p>
        <p className="mt-2 max-w-md text-sm leading-6 text-white/55">Apresentação visual preparada para receber mídia original do acervo da Rede Software.</p>
      </div>
    </div>
  )
}
...
function SectionHeading({ eyebrow, title, text }: { eyebrow: string; title: string; text?: string }) {
  return (
    <div className="max-w-3xl">
      <span className="text-xs font-black uppercase tracking-[.2em] text-[#e3262e]">{eyebrow}</span>
      <h2 className="mt-4 font-display text-4xl font-black leading-tight tracking-[-.05em] sm:text-5xl">{title}</h2>
      {text ? <p className="mt-5 text-lg leading-8 text-neutral-500">{text}</p> : null}
    </div>
  )
}
...
function InfoCard({ icon: Icon, title, text, dark = false }: Feature & { dark?: boolean }) {
  return (
    <article className={`interactive-surface rounded-[28px] border p-7 ${dark ? 'border-white/10 bg-white/5 text-white' : 'border-black/7 bg-white'}`}>
      <span className={`grid size-12 place-items-center rounded-2xl ${dark ? 'bg-white/10 text-red-300' : 'bg-red-50 text-[#e3262e]'}`}><Icon size={22} /></span>
      <h3 className="mt-8 font-display text-2xl font-black tracking-tight">{title}</h3>
      <p className={`mt-3 text-base leading-7 ${dark ? 'text-neutral-400' : 'text-neutral-500'}`}>{text}</p>
    </article>
  )
}
...
function PillButton({ to, children, secondary = false }: { to: string; children: ReactNode; secondary?: boolean }) {
  return secondary ? (
    <Link to={to} className="inline-flex items-center gap-2 rounded-xl border border-black/10 bg-white px-5 py-4 text-base font-bold text-[#101114] transition hover:border-black/15 hover:bg-neutral-50">{children}</Link>
  ) : (
...
function HomePage() {
  const cards = [
    { title: 'SGCI', eyebrow: 'GESTÃO COMERCIAL', text: 'Vendas, estoque, financeiro, fiscal e operação em um único ecossistema.', to: '/sgci', icon: BarChart3 },
    { title: 'SISTRANS', eyebrow: 'GESTÃO DE TRÂNSITO', text: 'Infrações, Talão Eletrônico, processos, arrecadação e informações para a gestão pública.', to: '/sistrans', icon: Landmark },
    { title: 'Certificação Digital', eyebrow: 'SEGURANÇA DIGITAL', text: 'Certificados para assinar documentos e realizar transações eletrônicas com validade jurídica.', to: '/certificacao-digital', icon: ShieldCheck },
  ]

  return (
    <main>
      <section className="hero-grid relative overflow-hidden">
        <div className="absolute -right-36 top-0 size-[620px] rounded-full bg-red-100/65 blur-3xl" />
        <div className="absolute left-[-18%] top-40 size-[360px] rounded-full bg-red-50 blur-3xl" />
        <div className="relative mx-auto grid min-h-[720px] max-w-7xl items-center gap-14 px-5 py-20 lg:grid-cols-[.92fr_1.08fr] lg:px-8 lg:py-24">
          <div>
            <span className="inline-flex rounded-full border border-red-100 bg-white px-4 py-2 text-xs font-black uppercase tracking-[.16em] text-[#e3262e]">Tecnologia que move negócios</span>
            <h1 className="mt-7 max-w-4xl font-display text-[clamp(3.4rem,6vw,6.6rem)] font-black leading-[.9] tracking-[-.075em]">Simplificando a <span className="text-[#e3262e]">gestão.</span><br />Conectando resultados.</h1>
            <p className="mt-7 max-w-2xl text-xl leading-8 text-neutral-500">Desenvolvemos soluções inteligentes para empresas e órgãos públicos trabalharem com mais eficiência, segurança e controle.</p>
            <div className="mt-9 flex flex-wrap gap-3"><PillButton to="/solucoes">Conheça nossas soluções <ArrowRight size={18} /></PillButton><PillButton to="/contato" secondary>Fale com um especialista</PillButton></div>
          </div>

          <div className="relative">
            <div className="absolute -inset-10 rounded-full bg-[radial-gradient(circle,rgba(227,38,46,.15),rgba(227,38,46,0)_65%)] blur-2xl" />
            <div className="relative overflow-hidden rounded-[34px] border border-black/10 bg-white p-4 shadow-[0_45px_120px_rgba(16,17,20,.14)]">
              <OriginalMedia search="SGCI" alt="SGCI Rede Software" className="aspect-[16/10] w-full rounded-2xl object-cover" fallback={<DashboardHeroVisual />} />
            </div>
            <div className="absolute -bottom-5 -left-5 hidden rounded-2xl border border-black/5 bg-white px-4 py-3 shadow-xl sm:block"><span className="text-xs font-black">Suporte humanizado</span><span className="mt-1 block text-sm text-neutral-500">Pessoas reais quando você precisa.</span></div>
          </div>
        </div>
      </section>

      <section className="relative z-10 -mt-5 px-5">
        <div className="mx-auto grid max-w-5xl gap-2 rounded-[26px] border border-black/6 bg-white p-3 shadow-[0_24px_70px_rgba(16,17,20,.08)] sm:grid-cols-2 lg:grid-cols-4">
          {[
            [Users, 'Clientes atendidos', 'Relacionamento próximo e experiência acumulada.'],
            [Zap, 'Soluções tecnológicas', 'Software, serviços e equipamentos conectados.'],
            [Headphones, 'Suporte especializado', 'Atendimento próximo e orientado por contexto.'],
            [ShieldCheck, 'Operação segura', 'Tecnologia pensada para o dia a dia.'],
          ].map(([Icon, title, text]) => <div key={title as string} className="rounded-2xl p-4"><Icon className="text-[#e3262e]" size={21} /><b className="mt-3 block text-base">{title as string}</b><span className="mt-1 block text-sm leading-6 text-neutral-500">{text as string}</span></div>)}
        </div>
      </section>

      <section className="mx-auto max-w-7xl px-5 py-28 lg:px-8">
        <SectionHeading eyebrow="SOLUÇÕES" title="Uma solução para cada desafio." text="Tecnologia para fazer tudo funcionar melhor, sem separar software, serviço e suporte em experiências desconectadas." />
        <div className="mt-12 grid gap-5 lg:grid-cols-3">
          {cards.map(({ title, eyebrow, text, to, icon: Icon }) => (
            <Link to={to} key={title} className="interactive-card group rounded-[32px] border border-black/7 bg-white p-7">
              <div className="flex items-center justify-between"><span className="text-xs font-black tracking-[.16em] text-[#e3262e]">{eyebrow}</span><span className="grid size-11 place-items-center rounded-xl bg-red-50 text-[#e3262e]"><Icon size={21} /></span></div>
              <div className="mt-7 overflow-hidden rounded-2xl bg-neutral-100"><OriginalMedia search={title} alt={title} className="aspect-[16/10] w-full object-cover" fallback={<FallbackVisual label={title} />} /></div>
              <h3 className="mt-7 font-display text-3xl font-black">{title}</h3><p className="mt-3 text-lg leading-8 text-neutral-500">{text}</p>
              <span className="mt-6 inline-flex items-center gap-2 text-base font-black text-[#e3262e]">Conhecer solução <ArrowRight size={17} className="transition group-hover:translate-x-1" /></span>
            </Link>
          ))}
        </div>
      </section>
...
function ProductLanding({ kind }: { kind: 'sgci' | 'sistrans' }) {
  ...
  const featureData: Feature[] = sgci ? [
    { icon: BarChart3, title: 'Retaguarda', text: 'Controle de estoque, financeiro, NF-e, SPED Fiscal, relatórios detalhados e informações centralizadas.' },
    { icon: ShoppingCart, title: 'Frente de Caixa', text: 'PDV integrado ao servidor, com suporte a operações fiscais e acompanhamento de estoque e contas a receber.' },
    { icon: CreditCard, title: 'Financeiro', text: 'Ajuste de preços, margens, descontos, multas, juros, parcelas e movimentações financeiras em um fluxo único.' },
    { icon: Package, title: 'Produtos e Estoque', text: 'Cadastro, composições, inventários, movimentações e relatórios para acompanhar produtos e serviços.' },
    { icon: ReceiptText, title: 'Relatórios', text: 'SPED, Sintegra, entradas, saídas, fluxo de caixa, comissões, contas a pagar e receber e indicadores gerenciais.' },
    { icon: ShieldCheck, title: 'Segurança', text: 'Armazenamento em ambiente seguro e acesso online para reduzir riscos de perda de dados e melhorar acessibilidade.' },
  ] : [
    { icon: FileCheck2, title: 'Gestão de Infrações', text: 'Acompanhe AITs, agentes, notificações, recursos, JARI e consultas detalhadas em um fluxo centralizado.' },
    { icon: Smartphone, title: 'Talão Eletrônico', text: 'Registre AITs em campo, inclusive em operação offline, com sincronização e recursos móveis.' },
    { icon: CircleDollarSign, title: 'Controle Financeiro', text: 'Organize arrecadações, repasses, restituições e baixas de pagamentos.' },
    { icon: BarChart3, title: 'Relatórios Gerenciais', text: 'Transforme os dados da operação em indicadores para acompanhamento e decisão.' },
    { icon: Landmark, title: 'Integrações', text: 'Conecte DEMUTRAN e DETRAN e acompanhe as etapas do processo de autuação e pagamento.' },
    { icon: ShieldCheck, title: 'Segurança e Permissões', text: 'Controle usuários, senhas, níveis de acesso e registros de operação.' },
  ]

  return <>
    <PageHero eyebrow={sgci ? 'SGCI · GESTÃO COMERCIAL' : 'SISTRANS · GESTÃO DE TRÂNSITO'} title={sgci ? <>Controle seu negócio. <span className="text-[#e3262e]">Simplifique sua gestão.</span></> : <>Gestão inteligente de <span className="text-[#e3262e]">infrações de trânsito.</span></>} text={sgci ? 'O SGCI simplifica a gestão empresarial integrando e automatizando processos para maior eficiência e controle.' : 'O SISTRANS gerencia infrações de trânsito e integra o DEMUTRAN com o DETRAN, apoiando autuação, processos e controle financeiro.'} imageSearch={sgci ? 'SGCI' : 'SISTRANS'}><div className="mt-8 flex flex-wrap gap-3"><PillButton to="/contato">Solicitar demonstração <ArrowRight size={17} /></PillButton><a href={WHATSAPP} target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 rounded-xl border border-black/10 bg-white px-5 py-4 text-base font-bold">Falar no WhatsApp</a></div></PageHero>
...
function ContactPage() {
  const channels = [
    { icon: Headphones, title: 'Suporte', text: 'Para clientes que precisam de orientação, atendimento e resolução de problemas.' },
    { icon: Zap, title: 'Comercial', text: 'Para conhecer soluções, solicitar demonstração e avaliar o que faz sentido para sua operação.' },
    { icon: Package, title: 'Produtos', text: 'Para consultar equipamentos, disponibilidade e soluções para varejo.' },
  ]
  ...
  return (
    <>
      ...
      <Routes>
        ...
        <Route path="/sgci" element={<ProductLanding kind="sgci" />} />
        <Route path="/sistrans" element={<ProductLanding kind="sistrans" />} />
        <Route path="/certificacao-digital" element={<CertificationPage />} />
